...
router.route('/generateWeeklyReport')
    .post(auth.adminProtect, adminControllers.generateWeeklyReport);


// Add grade calculator route
router.route('/gradeScale')
    .get(auth.adminProtect, (req, res) => {
        const adminControllers = require('../controllers/admin_controller');
        const gradeScale = adminControllers.getGradingSystem().getGradeScale();
